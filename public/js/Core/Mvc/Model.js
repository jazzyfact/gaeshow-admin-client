...
export default class Model {
        ...
        try {
            ...
            if (this.resStatus === 400)
                response.json().then((obj) => {
                    if (obj.code.includes('current_password_is_different')) throw  utils().snackbar(`기존 비밀번호가 틀림`), '클라오류 : 기존 비밀번호가 틀림.' 
                    if (obj.code.includes('both_passwords_are_the_same')) throw  utils().snackbar(`기존 비밀번호와 변경할 비밀번호가 같음`), '클라오류 : 기존 비밀번호와 변경할 비밀번호가 같음'
                    // if (obj.code.includes('both_passwords_are_the_same')) throw  
            ...
            if (this.resStatus === 404) return { stats: '404 Not Found' }
            if (this.resStatus === 405) throw '클라오류 : http method 찾을 수 없음'
            if (this.resStatus === 408) throw '클라오류 : 요청 시간 초과'
            if (this.resStatus === 409) {
                response.json().then((obj) => {
                    if (obj.code.includes('already_exist_profile_nickname')) throw '클라오류 : 이미 존재(가입)하는 유저입니다.'
                    if (obj.code.includes('check_process_status')) throw '클라오류 : 처리상태를 확인하세요.'
                    if (obj.code.includes('already_exist_comment')) throw '클라오류 :  이미 답변이 존재함'
                    if (obj.code.includes('already_exist_user')) throw '클라오류 :  : 중복 된 아이디'
    ...
    //delete에 body 넣기
    deleteBodyRequest = async (path, body) => {
        if (body) return await this.sendHttpRequest('DELETE', this.singleton.serverUrl + path  ,body)
        else return await this.sendHttpRequest('DELETE', this.singleton.serverUrl + path)
    }
    ...
    //개쇼서비스이미지보내기
    postRequestImageFormData = async (path, formData = new FormData()) => {
        return await this.sendHttpRequestForm('POST', this.singleton.gaeshowUrl + path, formData)
    }
}
