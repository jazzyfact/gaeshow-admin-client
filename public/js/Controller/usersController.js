...
// }
    ...
    getData = async () => {
        ...
        //1. 주소값의 ? 뒤에있는 필터값이 있는지 확인
        //2. 있다면 해당 값으로 필터 벨류값이 들어감
       // 3. 없다면 전체필터 값을 보냄
       //겟 파라미터값을 req.필터에 넣어줌!
  ...
  if (searchText) {
      ...
      this._searchInput.value = searchText
         ...
         //받아온유저정보
         const resData = await this._usersModel.getUsers(reqData)
    ...
    setReportItem = async (data) => {
        // console.log("받아온유저11122131",data)
 
        //엘리먼트에 있는 요소들을 제거 한 후 데이터 넣기
        const cell = this._view.getElement('#userTableSection')
        ...
        while (pageWrapper.hasChildNodes()) {
            ...
            const temp =   this._usersItem.getUser(item)
 ...
 deleteBtn = async (userId) =>{
    ...
    modal[3].onclick = async () => {
        ...
        const resData = await this._usersModel.delete(reqData)
        console.log("삭제reqData", reqData)

        if (resData) {
            utils().snackbar('삭제 되었습니다')
            document.body.removeChild(modal[0])
            window.location.href = `/index.html?p=${this._nowPage}`
        } else {
 ...
 recoverBtn = async (userId) =>{
    ...
    const dialog = window.confirm(`회원을 복구 하시겠습니까?!!`)
    if(dialog){
        // console.log('클릭')
            await this._usersModel.recover(userId)
...
setPagination = async (data) => {
    ...
    const paginationItem = paginationRepo.getUserPaging(data)
...
filterChangeEvent = async () => {
    ...
    window.location.href = `/index.html?filter=${this._pageFilterIndex}`
}
...
getEnterSearch = async (e) => {
 ...
 if(filter != ''){
     window.location.href = `/index.html?filter=${filter}&t=${type}&s=${text}`
 }
 else{
     window.location.href = `/index.html?t=${type}&s=${text}`
 }
}
    }
}
