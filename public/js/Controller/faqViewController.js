...
export default class faqViewController{
    ...
    lifeCycle = async () => {
        ...
        await this.getFaqData().then((e) => this.setFaqData(e))
    }
...
grade = async () => {
    ...
    switch(this._myGrade.grade){
     ...
     setFaqData = async (data) => {
        ...
        const deleteAt = data.deleted_at
        // console.log('삭제',deleteAt)
 ...
 modBtnClick = async (index) => {
    window.location.href = `/faq__edit.html?n=${index}`
}
...
deleteBtnClick = async (index) => {
    ...
    if (dialog) {
        const resData = await this._faqModel.deletePost(index)
        if (resData.stats == 'ok') {
