...
export default class userViewController{
    constructor(type,myGrade){
        ...
        //   console.log('회원 인덱스',this._userId)
          ...
          //제휴업체 등록
          this._addaffiliated = this._view.getElement('#affiliatedAddBtn')
          this._addaffiliated.onclick = () => this.affiliatedAddBtn()
    ...
    getData = async () =>{
        const resData =  await this._usersModel.getUserDetail(this._userId)
        //    console.log("가져온 광고 데이터", this._usersModel.getUserDetail(this._userId))
           return resData
       }
       ...
       setData = async (data) =>{
        ...
        //만약 제휴업체라면 버튼숨기기
        if(affiliated !== null){
            this._view.getElement('#affiliatedAddBtn').classList.add('hidden')
        }
        ...
        const lanView = this._view.getElement('#ides')
         //ide 태그 데이터 넣어주기
         ides.map((item) => {
            // const temp = this._postItem.createTagItem(item)
            // this._ides.appendChild(temp)
            // console.log('item',item)

         
            const element = document.createElement('p')
            element.classList.add('mypage__edit--subbox--item')
            element.innerHTML = item.name
            idesView.appendChild(element)
        })
        //언어 태그
        languages.map((item) => {
            // const temp = this._postItem.createTagItem(item)
            // this._languages.appendChild(temp)
            // console.log('item',item)
            ...
            element.classList.add('mypage__edit--subbox--item')
            element.innerHTML = item.name
            lanView.appendChild(element)
            
        })

        //탈퇴일 or 강퇴일 있다면 제휴업체 추가 버튼 숨기기
        if(deleted_at !== null){
            this._view.getElement('#affiliatedAddBtn').classList.add('hidden')
        }
