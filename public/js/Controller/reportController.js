...
import reportItem from '../View/reportItem.js'
...
export default class reportController{
    constructor(type,myGrade){
        ...
        this._reportItem = new reportItem()
        ...
        this._pageReasonFilterIndex = utils().getParameterByName('report_reason') ? utils().getParameterByName('report_reason') : 'all'
       ...
       //신고타입
        ...
        //신고이유
        this._reportReasonSelector = this._view.getElement('#reportReason')
        // this._reportReason.onchange = () => this.loadData()
        //일반 필터
    ...
    lifeCycle = async () => {
        await this.grade().then((e) => this.getData(e))
        ...
        //신고이유필터선택
        this._view.getElement('#reportReason').onchange = () => this.filterChangeEvent()
...
getData = async ()=>{
    ...
    reqData.service_category = this._report
    // console.log( "카테고리타입",reqData.service_category)


       
    const service_type = this._reportItem.getParameterByName('service_type')
    const report_reason  = this._reportItem.getParameterByName('report_reason')
    const filter = this._reportItem.getParameterByName('filter')
    const searchType = this._reportItem.getParameterByName('search_type')
    const searchWord = this._reportItem.getParameterByName('search_word')
    ...
    //신고이유선택
    if (report_reason && report_reason != 'default') reqData.report_reason = report_reason
    for (let i = 0; i < this._reportReasonSelector.options.length; i++) {
        if (this._reportReasonSelector.options[i].value == report_reason) {
            this._reportReasonSelector.options[i].selected = true
            // console.log("셀렉트옵션",  reqData.report_reason )   
        }
    }
    //일반필터 선택되게
 ...
 if (searchText) {
    ...
    //받아온 공지사항 정보
    const resData = await this._reportModel.getReportData(reqData)
    ...
    this.setReportItem(service_type, report_reason, filter)
}
...
setReportItem = async (service_type, report_reason, filter) => {

      //엘리먼트에 있는 요소들을 제거 한 후 데이터 넣기
      const cell = this._view.getElement('#reportTableSection')
      while (cell.hasChildNodes()) {
      cell.removeChild(cell.firstChild)
  }

  if(this._resData.inquire_count === 0 ){
    const item = this._reportItem.createEmptyListItem()
    ...
    while (pageWrapper.hasChildNodes()) {
        ...
        this._reportItem.getReportnItem(item)  
     })
    }
     this.setPagination(service_type, report_reason, filter)
 }
...
//페이징
setPagination = async (service_type, report_reason, filter) => {
    // console.log("직힘",service_type, report_reason, filter)
    const pageWrapper = this._view.getElement('#pagingData')
    ...
    const paginationItem = paginationRepo.getReportPaging(service_type, report_reason, filter)
 ...
 filterChangeEvent = async () => {
    ...
    this._pageReasonFilterIndex = this._view.getElement('#reportReason').value
   
    //문의타입필터랑 전체필터가 있다면 밑에 url주소 넘김
    if(this._pageFilterIndex != '' && this._pageTypeFilterIndex != '' && this._pageReasonFilterIndex != '')  window.location.href = `/report.html?service_type=${this._pageTypeFilterIndex}&report_reason=${this._pageReasonFilterIndex}&filter=${this._pageFilterIndex}`
}
 ...
 getEnterSearch = async (e) => {
    ...
    const report_reason = this._reportItem.getParameterByName('report_reason')
    const filter = this._reportItem.getParameterByName('filter')
    const search_type = this._reportItem.getParameterByName('t')
    ...
    if(e.key == 'Enter'){
     ...
     for (let i = 0; i < this._serviceSelector.options.length; i++) {
        if (this._serviceSelector.options[i].value == service_type) {
            ...
            // console.log("셀렉트옵션 this._serviceType.options[i].value", this._serviceType.options[i].value )   
        }
    }
    ...
    for (let i = 0; i < this._reportReasonSelector.options.length; i++) {
        if (this._reportReasonSelector.options[i].value == report_reason) {
            this._reportReasonSelector.options[i].selected = true
            // console.log("셀렉트옵션", report_reason  )   
            // console.log("셀렉트옵션 this._reportReason.options[i].value", this._categorySelector.options[i].value )   
        }
    }
    //일반필터 선택되게
 ...
 if(service_type != '' && filter != ''){
     window.location.href = `/report.html?service_type=${service_type}&report_reason=${report_reason}&filter=${filter}&t=${type}&s=${text}`
 }
 else{
     window.location.href = `/report.html?t=${type}&s=${text}`
 }
}
}
}
...
getSearch = async () => {
    ...
    //신고종류
     for (let i = 0; i < this._serviceSelector.options.length; i++) {
        if (this._serviceSelector.options[i].value == service_type) {
            this._serviceSelector.options[i].selected = true
            // console.log("셀렉트옵션", service_type )   
            console.log("셀렉트옵션 this._serviceType.options[i].value", this._serviceSelector.options[i].value )   
        }
    }
    ...
    for (let i = 0; i < this._reportReasonSelector.options.length; i++) {
        if (this._reportReasonSelector.options[i].value == report_reason) {
            this._reportReasonSelector.options[i].selected = true
            // console.log("셀렉트옵션", report_reason  )   
            console.log("셀렉트옵션 this._reportReason.options[i].value", this._reportReasonSelector.options[i].value )   
        }
    }
